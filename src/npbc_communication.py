...
class generalInformationResponse(successResponse):
  def __init__(self, data):
    ...
    self.Date = '{:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}'.format(2000 + int("{0:X}".format(data[7])), int("{0:X}".format(data[6])), int("{0:X}".format(data[5])),
...
class commandBase(object):
  ...
  def getRequestData(self, data):
    ...
    # header
    request[0:0] = bytearray(self.__header)
...
class setBoilerTemperatureCommand(commandBase):
  ...
  def processResponseData(self, response):
    ...
    if (self.IsSuccessful and len(responseData) == 1 and responseData[0] == 0x34):
